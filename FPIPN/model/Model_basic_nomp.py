...
class Config(object):
    ...
    kd = False           # False: tea only; True: tea and stu
...
def batch_sem_pooling(keys, w, b, q, tile):
    keys_len = keys.shape[-1]
    keys = tf.reshape(keys, [-1, keys_len])
    keys = tf.reshape(keys, [-1, tile, keys_len])
    keys = tf.reduce_sum(keys, 1)
    return keys
...
def batch_multi_node_sem_pooling(keys, w, b, q, tile, node_count):
    keys_len = keys.shape[-1]
    keys = tf.reshape(keys, [-1, keys_len])
...
def main():
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope('neighbor_attention'):
            ...
            paper_auth_agg = batch_attention(auth_feats_lp, paper_feat_lp, paper_auth_cnt, w_att_p_a, config.a_count)
            paper_topic_agg = batch_attention(topic_feats_lp, paper_feat_lp, paper_topic_cnt, w_att_p_t, config.t_count)
            paper_ref_agg = batch_attention(ref_feats_lp, paper_feat_lp, paper_ref_cnt, w_att_p_ref, config.ref_count)
            paper_ref_auth_agg = batch_attention(ref_auth_feats_lp, paper_feat_lp, paper_ref_auth_cnt, w_att_p_ref_a,
                                                 config.ref_a_count)
            paper_auth_paper_agg = batch_attention(auth_paper_feats_lp, paper_feat_lp, paper_auth_paper_cnt,
        ...
        with tf.name_scope('semantic_attention'):
            ...
            paper_embed = batch_sem_pooling(paper_agg, w_att_sem, b_att_sem, q_att_sem, 6)  # [bs,256]
            auths_embed = batch_multi_node_sem_pooling(auth_agg, w_att_sem, b_att_sem, q_att_sem, 3,
                                                   config.a_count)  # [bs,6,256]
            topics_embed = batch_multi_node_sem_pooling(topic_agg, w_att_sem, b_att_sem, q_att_sem, 3,
        ...
        with tf.Session(config=config_gpu) as sess:
            ...
            train_log = open('year'+str(config.years)+'/basic_train_log_fold_nomp_'+str(config.fold)+'.txt', 'w+')
            train_log.write(datetime.datetime.now().strftime('%F %T') + '\n')

            train_log_stu = open('year'+str(config.years)+'/basic_train_log_stu_nomp'+str(config.T)+'.txt', 'w+')
