...
def main():
    ...
    with tf.Session(config=config_gpu) as sess:
        ...
        train_result_stu = open('year'+str(config.years)+'/train_result_stu_'+'fold'+str(config.fold) + "_" + str(config.T) + '.txt', 'w+')
        train_result_stu.write(datetime.datetime.now().strftime('%F %T') + '\n')
        ...
        for i in range(config.epochs):
            ...
            try:
                while True:
                    ...
                    batch_acc = sess.run(acc_stu, feed_dict=test_feed_dict)
                    for k in range(5):
                        test_acc_stu[k] += batch_acc[k]

                    pre, gt = sess.run([stu_prediction_exp, citation_exp], feed_dict=test_feed_dict)
                    train_result_stu.write('precision : ' + str(pre) + '\n')
                    train_result_stu.write('ground truth : ' + str(gt) + '\n')
                ...
                for k in range(5):
                    test_acc_stu[k] /= test_counts
                train_log_stu.write(
                    'stu epoch : ' + str(i + 1) + ' testing loss: ' + str(test_loss_gt) + ' testing acc: ' + str(
                        test_acc_stu) + '\n')

                train_result_stu.write('----------------------------' + '\n')
