...
def batch_bilstm(x, cnt, h_size, tile_size, scope):  # x [bs, tile_size, embed_size]
    with tf.variable_scope(scope + '_fw'):
        fw_cell = rnn.BasicLSTMCell(h_size, forget_bias=1.0)
    with tf.variable_scope(scope + '_bw'):
        bw_cell = rnn.BasicLSTMCell(h_size, forget_bias=1.0)

    embed_size = int(x.shape[-1])
    with tf.variable_scope(scope + '_weight'):
        weight = tf.get_variable('w_bilstm', [2 * h_size, embed_size], tf.float32, get_init(2 * h_size, embed_size))
    x = tf.unstack(x, tile_size, axis=1)  # x [tile_size, bs, embed_size]
    with tf.variable_scope(scope + '_bilstm'):
        output, fw, bw = rnn.static_bidirectional_rnn(fw_cell, bw_cell, x,
                                                      dtype=tf.float32)  # output [tile_size, bs, 2*h_size]
    output = tf.matmul(output, weight)
    output = tf.reduce_sum(output, 0) / cnt  # output [bs,embed_size]
    return output


def batch_multi_node_bilstm(x, cnt, h_size, tile_size, node_count, scope):
    with tf.variable_scope(scope + '_fw'):
        fw_cell = rnn.BasicLSTMCell(h_size, forget_bias=1.0)
    with tf.variable_scope(scope + '_bw'):
        bw_cell = rnn.BasicLSTMCell(h_size, forget_bias=1.0)
    ...
    with tf.variable_scope(scope + '_weight'):
        weight = tf.get_variable('w_bilstm', [2 * h_size, embed_size], tf.float32, get_init(2 * h_size, embed_size))
    x = tf.reshape(x, [-1, node_count, tile_size, embed_size])  # x [bs, node, tile, embed]
    x = tf.transpose(x, [2, 0, 1, 3])  # x [tile, bs, node, embed]
    x = tf.reshape(x, [tile_size, -1, embed_size])  # x [tile, bs*node, embed]
    x = tf.unstack(x, tile_size, axis=0)  # x tile * [bs*node, embed]
    with tf.variable_scope(scope + '_bilstm'):
        output, fw, bw = rnn.static_bidirectional_rnn(fw_cell, bw_cell, x,
                                                      dtype=tf.float32)  # output [tile_size, bs*node_count, 2*h_size]
    output = tf.reshape(output, [tile_size, -1, node_count, 2 * h_size])
    output = tf.matmul(output, weight)
    output = tf.reduce_sum(output, 0)  # output [bs, node_count, embed_size]

    op_len = output.shape[-1]
    output = tf.reshape(output, [-1, op_len])
    cnt = tf.reshape(cnt, [-1, 1])
    output = output / cnt

    output = tf.reshape(output, [-1, node_count, op_len])
    return output
...
def batch_attention(keys, query, cnt, weight, tile_size):
    ...
    return res_keys  # res_keys [bs, embed_size]
...
def batch_multi_node_attention(keys, query, cnt, weight, tile_size, node_count):
    ...
    res_keys = tf.nn.tanh(res_keys)
    return res_keys  # res_keys [bs, node_count, embed_size]
...
def main():
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope('neighbor_bilstm'):
            paper_auth_agg = batch_bilstm(auth_feats_lp, paper_auth_cnt, config.bilstm_length, config.a_count, 'p_a')
            paper_topic_agg = batch_bilstm(topic_feats_lp, paper_topic_cnt, config.bilstm_length, config.t_count, 'p_t')
            paper_ref_agg = batch_bilstm(ref_feats_lp, paper_ref_cnt, config.bilstm_length, config.ref_count, 'p_p')
            paper_ref_auth_agg = batch_bilstm(ref_auth_feats_lp, paper_ref_auth_cnt, config.bilstm_length,
                                              config.ref_a_count, 'p_p_a')
            paper_auth_paper_agg = batch_bilstm(auth_paper_feats_lp, paper_auth_paper_cnt,
                                                config.bilstm_length, config.a_p_count, 'p_a_p')
            ...
            auth_paper_agg = batch_multi_node_bilstm(auth_paper_feats_lp, auth_p_cnt, config.bilstm_length,
                                                     config.p_tile, config.a_count, 'a_p')
            auth_coa_agg = batch_multi_node_bilstm(coauth_feats_lp, auth_coauth_cnt, config.bilstm_length,
                                                   config.coa_tile, config.a_count, 'a_p_a')
            ...
            topic_paper_agg = batch_multi_node_bilstm(topic_paper_feats_lp, topic_p_cnt, config.bilstm_length,
                                                      config.p_tile, config.t_count, 't_p')
            topic_auth_agg = batch_multi_node_bilstm(topic_paper_auth_feats_lp, topic_p_a_cnt,
                                                     config.bilstm_length,
                                                     config.t_a_tile, config.t_count, 't_p_a')
        ...
        with tf.name_scope('predict_attention'):
            ...
            auth_paper_pre = batch_pooling(auths_embed, paper_embed, paper_auth_cnt, w_att_a_p_pre, config.a_count)

            w_att_a_t_pre = tf.get_variable('w_att_a_t_pre', [config.lp_length * 2, 1], tf.float32,
                                            get_init(config.lp_length * 2, 1))

            auth_topic_pre = batch_pooling(auths_embed, topics_embed_mean, paper_auth_cnt, w_att_a_t_pre,
                                           config.a_count)
        ...
        with tf.Session(config=config_gpu) as sess:
            ...
            train_log = open('year' + str(config.years) + '/train_log_hetgnn_fold' + str(config.fold) + '.txt',
                             'w+')
            train_log.write(datetime.datetime.now().strftime('%F %T') + '\n')

            train_log_stu = open('year' + str(config.years) + '/train_log_hetgnn_stu_' + str(config.T) + '.txt',
                                 'w+')
            ...
            if not config.no_tea:
                ...
                for i in range(config.epochs):
                    ...
                    # saver.save(sess, 'year'+str(config.years)+'/ckpt/Model_fold'+str(config.fold)+'.ckpt')
                    ...
                    try:
                        ...
                        train_log.write(
                            'epoch: ' + str(i + 1) + ' testing loss: ' + str(test_loss) + ' testing acc: ' + str(
                                test_acc) + '\n')
                        ...
                        try:
                            while True:
                                ...
                                if train_batch_count % 100 == 0:
                                    ...
                                    train_log_stu.write(
                                        'stu epoch : ' + str(i + 1) + ' batch: ' + str(
                                            train_batch_count) + ' loss: ' + str(
