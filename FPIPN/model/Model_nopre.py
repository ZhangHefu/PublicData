...
def main():
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config_gpu) as sess:
            ...
            train_log = open('year'+str(config.years)+'/train_log_no_pre_fold'+str(config.fold)+'.txt', 'w+')
            train_log.write(datetime.datetime.now().strftime('%F %T') + '\n')

            train_log_stu = open('year'+str(config.years)+'/train_log_stu_no_pre_'+str(config.T)+'.txt', 'w+')
