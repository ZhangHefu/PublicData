...
def main():
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config_gpu) as sess:
            ...
            train_log = open('year'+str(config.years)+'/train_log_noneigh_fold'+str(config.fold)+'.txt', 'w+')
            train_log.write(datetime.datetime.now().strftime('%F %T') + '\n')

            train_log_stu = open('year'+str(config.years)+'/train_log_noneigh_stu_'+str(config.T)+'.txt', 'w+')
            ...
            if not config.no_tea:
                ...
                for i in range(config.epochs):
                    ...
                    saver.save(sess, 'year'+str(config.years)+'/ckpt/Model_noneigh_fold'+str(config.fold)+'.ckpt')
